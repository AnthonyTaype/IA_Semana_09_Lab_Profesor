...
public class Mochila extends JFrame implements ActionListener {
	...
	void calcularDescuento(){
		if(cant<5){impD=impC*0.0;}
		else if(cant>=5 && cant<7){impD=impC*0.05;}
		else if(cant>=7 && cant<9){impD=impC*0.07;}
		else{impD=impC*0.09;}	
	}
	void calcularImpPagar(){
		impP = impC-impD;
    }
	void calcularChocolates(){
		if(impP<200){ chocolate = 0;}
		else if(impP>=200 && impP<300){chocolate= 1*cant;}
		else if(impP>=500 && impP<700){chocolate= 2 * cant;}
		else{chocolate= 3 * cant;}
	}
	void mostrarResultados(){
		// Salida de resultados
				...
				txtS.append ("Chocolate         : " + chocolate + "\n");
